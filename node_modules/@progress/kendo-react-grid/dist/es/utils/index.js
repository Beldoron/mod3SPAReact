 ...
 * @hidden
 */
export function flatData(output, input, groups, footer, field, level) {
    ...
    if (level >= groups) {
        for (var q = 0; q < input.length; q++) {
            output[output.length] = {
                ...
                rowType: 'data',
                level: level,
                expanded: field === undefined || getNestedValue(field, input[q])
            };
        }
        return;
    }
    for (var i = 0; i < input.length; i++) {
        ...
        // children
        if (expanded) {
            flatData(output, input[i].items, groups, footer, field, level + 1);
        }
        // footer
